        ...
        for data in r.iter_content(chunk_size=2500*1024):
            ...
            if current_progress != previows_progress:
                ...
                if download_speed >= 1:
                    ...
                    speed_txt = "KB/s"
                print(f"Tập tin tải về {filename} {current_progress}% kết thúc ({download_speed:.2f} {speed_txt} / {total_txt})")
...
def run_lavalink(
    ...
    print(f"Khởi động máy chủ Lavalink (tùy thuộc vào hosting mà Lavalink có thể mất một lúc để khởi động," 
          f"Xin lưu ý rằng lỗi có thể xảy ra trong một số lần thử kết nối trước khi bắt đầu hoàn toàn..\n{'-' * 30}")

    lavalink_process = subprocess.Popen(java_cmd.split(), stdout=subprocess.DEVNULL)

    if lavalink_additional_sleep:
        print(f"Chờ đợi {lavalink_additional_sleep} giây...\n{'-' * 30}")
