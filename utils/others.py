...
class PlayerControls:
    ...
    keep_connected = "musicplayer_keepconnected"
    ...
    async def interaction_check(self, interaction: disnake.MessageInteraction) -> bool:
        ...
        if interaction.author != self.ctx.author:
            await interaction.send(
                f"Chỉ thành viên {self.ctx.author.mention} mới có thể sử dụng các nút trong tin nhắn này...",
                ephemeral=True
            )
            return False
...
song_request_buttons = [
    ...
    disnake.ui.Button(label="Chơi yêu thích/tích hợp ", emoji="⭐", custom_id=PlayerControls.enqueue_fav)
]
...
async def send_idle_embed(
    ...
    if text:
        embed.description += f"**Tương tác cuối cùng:** {text.replace('**', '')}\n"
    ...
    if opts:
        components.append(
            disnake.ui.Select(
                placeholder="Bài hát/Danh sách phát từ máy chủ.",
    ...
    if is_forum:
        content = "🎶 Yêu cầu âm nhạc của bạn ở đây."
    else:
        content = None
...
async def select_bot_pool(inter: Union[CustomContext, disnake.MessageInteraction, disnake.AppCmdInter], first=False, return_new=False, edit_original=False):
    ...
    if len(bots) == 1 or first:
        ...
        try:
            ...
            try:
                await msg.edit(conent="Đã hết thời gian chọn!", embed=None, view=None)
            except:
                pass
            return None, None
        ...
        if new_inter.data.values[0] == "cancel":
            await func(
                embed=disnake.Embed(
                    description="**Lựa chọn đã bị hủy**",
                    color=inter.bot.get_color()
                ),
                components=None
            )
        ...
        try:
            ...
            raise GenericError("**Bot được chọn đã bị xóa khỏi máy chủ trước khi được chọn...**")
