...
class IndexHandler(tornado.web.RequestHandler):
    ...
    async def get(self):
        ...
        if killing_state is True:
            self.write('<h1 style=\"font-size:5vw\">Một ứng dụng tuyệt vời sẽ xuất hiện...</h1>')
            return
        ...
        for bot in sorted(self.pool.bots, key=lambda b: b.identifier):
            ...
            if bot.is_ready():
                ...
                ready_bots.append(
                    ...
                    f"Thêm bot :>:<br><a href=\"{disnake.utils.oauth_url(bot.user.id, permissions=disnake.Permissions(bot.config['INVITE_PERMISSIONS']), scopes=('bot', 'applications.commands'), **kwargs)}\" "
                    f"rel=\"nofollow\" target=\"_blank\">{bot.user}</a>" + (f"<br>Số máy chủ hiện tại: {guilds}" if guilds else "") + "</td></tr>"
                )
            else:
                pending_bots.append(f"<tr><td>{bot.identifier}</td></tr>")

        if ready_bots:
            msg += f"\n<p style=\"font-size:20px\">Các bot đã sẵn sàng:</p>" \
                   f"{style}\n<table cellpadding=\"3\">{''.join(ready_bots)}</table>"

        if pending_bots:
            msg += f"\n<p style=\"font-size:20px\">Bot đang khởi động:</p>" \
                   f"{style}\n<table cellpadding=\"10\">{''.join(pending_bots)}</table>\n" \
                   f"Lưu ý: Tải lại trang để kiểm tra xem bot đã có chưa."
        ...
        if failed_bots:
            ...
            msg += f"\n<p style=\"font-size:20px\">Các mã thông báo sau được định cấu hình trong ENV/SECRET/.env không thành công" \
        ...
        if self.config["ENABLE_RPC_AUTH"]:
            msg += f"\nĐừng quên lấy mã thông báo để định cấu hình trong ứng dụng, hãy sử dụng lệnh /rich_presence để nhận mã thông báo.\n<br><br>"
        msg += f"\nTiền tố mặc định: {self.pool.config['DEFAULT_PREFIX']}<br><br>"
