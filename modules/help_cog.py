    ...
    def process_buttons(self):
        ...
        for category, emoji in self.items:
            ...
            b = disnake.SelectOption(
                ...
                description="Xem chi tiết các lệnh trong danh mục này."
            )

            options.append(b)

        if options:
            sel = disnake.ui.Select(placeholder='Chọn một danh mục để xem tất cả các lệnh:', options=options)
        ...
        if self.category:
            ...
            back_button = disnake.ui.Button(style=disnake.ButtonStyle.grey, emoji='<:leftwards_arrow_with_hook:868761137703964692>', custom_id="back_page", label="Quay trở lại")
...
class HelpCog(commands.Cog, name="Hỗ trơ lệnh"):
    ...
    async def get_cmd(self, ctx, cmds, index=0, category=None, emoji=None):
        ...
        if cmd.aliases:
            ...
            txt += f"🔄 **⠂Các lựa chọn thay thế:** ```\n{aliases}```\n"
        if hasattr(cmd, 'commands'):
            subs = " | ".join([c.name for c in cmd.commands if (await check_perms(ctx, c))])
            txt += f"🔢 **⠂Lệnh phụ:** ```{subs}``` Sử dụng lệnh: `[ {ctx.prefix}help {cmd} subcomando ]` để xem thêm chi tiết về lệnh phụ.\n\n"
        ...
        if flags and (actions := flags._actions):
            ...
            if t:
                txt += ("🚩 **⠂Cờ ` (tùy chọn thêm vào cuối lệnh)`:**```ini\n" + "\n\n".join(t) + "```")

        embed.set_author(name="Menu trợ giúp - Danh sách lệnh (prefix)", icon_url=self.bot.user.display_avatar.url)
    ...
    async def _help(self, ctx, *cmd_name):
        ...
        for cmd in sorted(ctx.bot.commands, key=lambda c: c.name):
            ...
            if category_icon:
                ...
                if not cmd.cog.emoji:
                    cmd.cog.emoji = "⁉"
                    cmd.cog.name = "Không có danh mục"
        ...
        txt = f"{''.join(lst)}\n\n" \
              "Để lấy thông tin trực tiếp từ một lệnh, hãy sử dụng: \n" \
        ...
        embed.set_author(name=f"Menu trợ giúp - Danh sách lệnh (prefix)",
    ...
    async def parse_direct(self, ctx: CustomContext, cmd_name: list):
        ...
        if any(c for c in cmd.cog.get_commands() if check_cmd(c)):
            ...
            name = cog.name if hasattr(cog, "name") else "Một số lệnh"
